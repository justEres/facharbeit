...
#let codly-init(
  ...
  show raw.where(block: true): it => context {
    ...
    for (i, line) in it.lines.enumerate() {
      ...
      // Use the original raw line text instead of highlighted children so
      // wrapped continuation lines always align with the source indentation.
      if smart-indent and line.has("text") {
        let match = line.text.match(indent_regex)

        // Ensure there is a match and it starts at the beginning of the line.
        if match != none and match.start == 0 {
          let indent = line.text.slice(match.start, match.end)
